...
function Get-PsqlCommand {
  ...
  throw "psql.exe nao foi encontrado. Instale o PostgreSQL client ou adicione psql ao PATH."
}

function Ensure-Postgres {
  $service = Get-PostgresService
  if (-not $service) {
    Write-Host "PostgreSQL: servico nao encontrado. Vou assumir que ja esta disponivel por outro meio."
    return
  }

  if ($service.Status -ne "Running") {
    Write-Host "PostgreSQL: iniciando servico $($service.Name)..."
...
function Ensure-RedisInstalled {
  ...
  if (-not $scoop) {
    throw "Redis nao esta instalado e Scoop nao foi encontrado. Instale Redis ou Scoop e rode scripts\setup-sorteio.bat."
  }
  ...
  $redis = Get-Command redis-server -ErrorAction SilentlyContinue
  if (-not $redis) {
    throw "Redis foi instalado, mas redis-server ainda nao esta no PATH. Abra um novo terminal e tente novamente."
  }
...
function Ensure-Database {
  ...
  if ($exists -eq "1") {
    Write-Host "PostgreSQL: banco sorteio ja existe."
    return
  }
...
function Start-Redis {
  ...
  if (Test-Port 6379) {
    Write-Host "Redis: ja esta escutando em localhost:6379."
    return
  }

  $existingPid = Get-AlivePid $RedisPidFile
  if ($existingPid) {
    Write-Host "Redis: processo ja registrado com PID $existingPid."
    return
  }
  ...
  if (-not (Wait-Port 6379 15)) {
    throw "Redis foi iniciado, mas nao respondeu em localhost:6379."
  }
...
function Start-Worker {
  ...
  if ($existingPid) {
    Write-Host "Worker: ja esta rodando com PID $existingPid."
    return
  }
...
function Start-App {
  ...
  if (-not (Wait-Port $port 30)) {
    throw "Web App foi iniciado, mas nao respondeu em $url."
  }
...
function Invoke-Setup {
  ...
  $npm = Get-NpmCommand
  Write-Host "Node: instalando dependencias..."
  & $npm install
  if ($LASTEXITCODE -ne 0) {
